...
export const VlyToolbar: React.FC = () => {
  ...
  const [bannerRendered, setBannerRendered] = React.useState(true);
  React.useEffect(() => {
    if (!showBanner) {
      // Wait for animation to finish before removing from DOM
      const timeout = setTimeout(() => setBannerRendered(false), 250);
  ...
  return (
    <>
      {isDevDeployment && bannerRendered && (
        <div
          style={{
            position: "fixed",
            top: 0,
            ...
            padding: showBanner ? "14px 10px 12px 10px" : "0 10px 0 10px",
            overflow: "hidden",
          }}
        >
          <span
          ...
          >
            ⚠️ This is a <b>DEV deployment</b>. Do not share this link for
            production use.
          </span>
          <button
